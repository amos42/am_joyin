...
#define MAX_MCP3008_ADC_CHANNEL_COUNT  (8)
#define MAX_MCP3008_ADC_VALUE          (1023)
...
typedef struct tag_device_mcp3008_config {
    ...
    int sampling_count;
} device_mcp3008_config_t;

typedef struct tag_device_mcp3008_index_item {
    int adc_channel;
    int abs_input;
    int min_value;
    int max_value;
    ...
    int max_adc_value;
    int mid_adc_value;
} device_mcp3008_index_item_t;

typedef struct tag_device_mcp3008_index_table {
    device_mcp3008_index_item_t buttondef[MAX_INPUT_BUTTON_COUNT];
    int pin_count;
    int button_start_index;
} device_mcp3008_index_table_t;
...
#define INPUT_MCP3008_DEFAULT_KEYCODE_TABLE_ITEM_COUNT  (8)

static const device_mcp3008_index_table_t default_input_mcp3008_config = {
    {
        ...
        {4, ABS_THROTTLE, -DEFAULT_INPUT_ABS_MAX_VALUE, DEFAULT_INPUT_ABS_MAX_VALUE, 0, MAX_MCP3008_ADC_VALUE, MAX_MCP3008_ADC_VALUE/2},
        {5, ABS_WHEEL,    -DEFAULT_INPUT_ABS_MAX_VALUE, DEFAULT_INPUT_ABS_MAX_VALUE, 0, MAX_MCP3008_ADC_VALUE, MAX_MCP3008_ADC_VALUE/2},
        {6, ABS_HAT0X,    -DEFAULT_INPUT_ABS_MAX_VALUE, DEFAULT_INPUT_ABS_MAX_VALUE, 0, MAX_MCP3008_ADC_VALUE, MAX_MCP3008_ADC_VALUE/2},
        {7, ABS_HAT0Y,    -DEFAULT_INPUT_ABS_MAX_VALUE, DEFAULT_INPUT_ABS_MAX_VALUE, 0, MAX_MCP3008_ADC_VALUE, MAX_MCP3008_ADC_VALUE/2}
    }, 
    INPUT_MCP3008_DEFAULT_KEYCODE_TABLE_ITEM_COUNT,
    0
};


// device 파라미터 파싱
static int __parse_device_param_for_mcp3008(device_mcp3008_data_t* user_data, char* device_config_str)
{
    char szText[512];
    char* pText;
    ...
    if (device_config_str != NULL) {
        ...
        user_data->device_cfg.spi_channel = parse_number(&pText, ",", 0, MCP3008_DEFAULT_SPI_CHANNEL);
        ...
        user_data->device_cfg.spi_channel = MCP3008_DEFAULT_SPI_CHANNEL;
...
// 각 endpoint 파라미터 파싱
static int __parse_endpoint_param_for_mcp3008(device_mcp3008_data_t* user_data, char* endpoint_config_str[], input_endpoint_data_t *endpoint_list[], int endpoint_count)
{
    int i, j;
    int code_mode;
    device_mcp3008_index_table_t *src, *des;
    char szText[512];
    char* pText;
    char* temp_p;
    ...
    for (i = 0; i < endpoint_count; i++ ) {
        ...
        if (cfgtype_p == NULL || strcmp(cfgtype_p, "default") == 0 || strcmp(cfgtype_p, "") == 0){
            src = (device_mcp3008_index_table_t *)&default_input_mcp3008_config;
            ...
            while (pText != NULL && pin_count < MAX_INPUT_BUTTON_COUNT) {
                ...
                // 키 설정 추가
                if (adc_channel >= 0 && abs_input >= 0 && adc_channel < MAX_MCP3008_ADC_CHANNEL_COUNT) {
                    ...
                    src->buttondef[pin_count].max_adc_value = parse_number(&block_p, ",", 10, MAX_MCP3008_ADC_VALUE);
                    src->buttondef[pin_count].mid_adc_value = parse_number(&block_p, ",", 10, MAX_MCP3008_ADC_VALUE / 2);
                    pin_count++;
                }
            }
        } else {
...
static int init_input_device_for_mcp3008(input_device_data_t *device_data, char* device_config_str, char* endpoint_config_strs[])
{
    ...
    result = __parse_endpoint_param_for_mcp3008(user_data, endpoint_config_strs, device_data->target_endpoints, device_data->target_endpoint_count);
    if (result != 0) {
        kfree(user_data);
        return result;
    }

    for (i = 0; i < MAX_MCP3008_ADC_CHANNEL_COUNT; i++) {
...
static void start_input_device_for_mcp3008(input_device_data_t *device_data)
{
    // device_mcp3008_data_t *user_data = (device_mcp3008_data_t *)device_data->data;
...
static void check_input_device_for_mcp3008(input_device_data_t *device_data)
{
    int i, j, k, cnt;
    int spi_channel;
    ...
    for (i = 0; i < device_data->target_endpoint_count; i++) {
        ...
        device_mcp3008_index_table_t* table = &user_data->button_cfgs[i];
        device_mcp3008_index_item_t* btndef = &table->buttondef[table->button_start_index];
...
static void stop_input_device_for_mcp3008(input_device_data_t *device_data)
{
