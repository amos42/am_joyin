...
// device_config_str : read_gpio, {addr0_gpio, addr1_gpio, ...}, cs_gpio, io_count
...
static int init_input_device_for_mux(void* device_desc_data, input_device_data_t *device_data, char* device_config_str, char* endpoint_config_strs[])
{
...
static void start_input_device_for_mux(input_device_data_t *device_data)
{
    ...
    for (i = 0; i < user_data->device_cfg.addr_bit_count; i++) {
        gpio = user_data->device_cfg.addr_gpio[i];
        if (gpio != -1) {
            gpio_as_output(gpio);
        }
    }
